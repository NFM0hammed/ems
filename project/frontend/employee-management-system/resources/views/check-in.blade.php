...
@section("content")
    <div class="container">
        <div class="form">
            ...
            <form class="bg-white row g-3 mt-4 mb-4 p-3 border-top" method="POST" action="{{ route("user.checkin_test") }}">
                @if ($errors->has("id_error"))
                    <p class="bg-danger text-white p-2">{{ $errors->first("id_error") }}</p>
                @endif
                @if ($errors->has("check_in_exists"))
                    <p class="bg-warning text-white p-2">{{ $errors->first("check_in_exists") }}</p>
                @endif
                @csrf
                <div>
